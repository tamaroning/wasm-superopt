...
| **B: 遷移表・オペランドの e-class 閉包** | §2.3 の $T_\oplus/T_\circ$ と §11.3 の $\mathit{in}[k]$ 固定を、代表元でなく $\equiv_R$ 同値類（e-class）全体で閉じる。binop の全オペランド順エッジを許容し、可換順差を吸収。A が供給した同値を探索空間に反映 | §2.3, §5.3, §11.3 | Denali（`args(T)` = e-class, `B(i,Q)`） |
...
**符号化と再検証の整合**（§11.6）は A・B の前提: CNF が許すスタック値と再検証が要求する不透明入力の同値判定を、同一の $\equiv_R$ に揃える。優先順は **A → B**（主因の可換オペランド順を潰す連鎖）、次いで原理的な **C・D**、実務的な **E**、正直さの **F**。詳細な現象分析と依存関係は [`0701_suboptimal_cause.md`](0701_suboptimal_cause.md) §5 を参照。
