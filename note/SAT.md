...
| **$T_\oplus, T_\circ$ に未定義の遷移が必要なスケジュール** | $V$ 拡張に伴う $T$ 再計算、または遷移の動的追加 |
...
- `local.tee` による set/get 融合と、call を挟んだ spill タイミング変更の**組み合わせ**
- オリジナルトレースに無かった中間スタック構成を経由するデータフロー
...
| $T$ が事前計算の遷移のみ許可し、新データフローの演算ペアを拒否 | §2.3, §5.3 |
...
> **既知のギャップ（§7.5, §11.8）:** `local.tee` によるスケジュール変更後、load / call 等のオペランドは実行上は同値でも、元トレースの $\mathit{in}[k]$ と異なる構文木になりうる（典型: 可換 `add` のオペランド順）。この場合、§11.3 の要求は探索空間を過剰に狭め、tee 融合解を表現不能にしうる。対処は §11.8 を参照。
...
§3 に $\mathrm{tee}_x$ があるにもかかわらず SuperStack 型の短縮に届かない場合の手法レベルの対処:

| 方針 | 内容 | 関連節 |
| --- | --- | --- |
| **オペランド同値類への緩和** | §11.3 の $\mathit{in}[k]$ 固定を、記録シンボルと $\equiv_R$ 同値な式のクラス全体に拡張。可換演算のオペランド順差を吸収 | §11.3, §11.6 |
| **符号化と再検証の整合** | CNF が許すスタック値と、再検証が要求する不透明入力の同値判定を同一の $\equiv_R$ 前提に揃える | §11.6 |
| **語彙の CEGAR 的拡張** | UNSAT や再検証失敗時に、候補短縮スケジュールから得られる中間値を $V$ に追加して再エンコード（§10） | §2.1, §10 |
| **call 境界のスタック保持** | call 前後でローカル退避の代わりにスタック直結する系列を、$H$ と call の $(p,q)$ 制約の下で明示的に許容する設計を検討 | §11.3, §11.5 |
| **最適性ラベルの分離** | 「符号化内で $L^*-1$ が UNSAT」と「グローバル最適」を出力上区別（§6.4 の `is_proven_optimal` を符号化相対と明示） | §6, §7.2 |

詳細な現象分析は [`0701_suboptimal_cause.md`](0701_suboptimal_cause.md) を参照。
